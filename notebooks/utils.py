...
def check_empty_field(field_name, data_elements):
    ...
    print(f"Data elements with missing {field_name}:")
    return empty_fields
...
def check_field_type(data_elements):
    allowed_types = [
        "text",
        "integer",
        "float",
    ...
    wrong_type = data_elements[~data_elements["Field Type"].isin(allowed_types)]
    if wrong_type.shape[0] == 0:
        print("Field Types: ok")
        return

    print(f"Field Type must be one of the following: {allowed_types}")
    print(f"Data elements with invalid Field Type:")
    return wrong_type
...
def download_csv_and_and_parse_column_names(csv_dir_path_name):
    import os
    import glob
    import pandas as pd
